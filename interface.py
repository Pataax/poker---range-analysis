...
root.title('Poker Range Analysis')

range_dict = {'Pré-Flop':{}, 'Flop':{}, 'Turn':{}, 'River':{}}

def creates_frame_streets_buttons(frame, street, qtd, tab_name, row, column):
    global range_dict

    frame_buttons = tkinter.Frame(root)
    frame_buttons.grid(row = row, column = column, sticky = 's', ipady = 7)

    for n in range(qtd):
        button_name = f'{tab_name}{n + 1}'
...
# creates_frame_streets_buttons(main_frame, street, path['qtd'], path['tab_name'], 0, 0)
